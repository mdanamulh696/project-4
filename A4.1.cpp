...
void printHousehold(Household* house, float pricePerKWh, int number_of_household) 
{
...
void readCityInformation(ifstream& ifs, string& taskIdentifier, char separator, int& length, string& city, float& priceKilowattPerHour)
{
    ...
    // Extract price per kilowatt per hour
    while (i < line.length())
    {
        curr += line[i];
        i++;
    }
    priceKilowattPerHour = stof(curr);
}

void readHouseholdInformation(string line, char separator, int startIndex, int& index, string& city, bool& hotWater, int& numberOfPeople, int& squareMeters)
{
    // Extract index
    string curr = "";
    int i = startIndex;
...
void readDataFromFile(string fileName, string& taskIdentifier, char separator, Household**& households, int n, int& length, string& city, float& priceKilowattPerHour)
{
    // Open the input file
    ...
    readCityInformation(ifs, taskIdentifier, separator, length, city, priceKilowattPerHour);
    ...
    else
    {
        ...
        while (getline(ifs, line))
        {
            ...
            if (curr == "household")
            {
                ...
                readHouseholdInformation(line, separator, i, index, house->city, house->Hot_water, house->numPersons, house->squareMeters);
...
void writeDataToFile(string fileName, char separator, int n, string city, float price, Household** households)
{
    ...
    ofs << "A4" << separator << to_string(n) << separator << city << separator << to_string(price) << '\n';
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (households[i])
        {
            ...
            if (households[i]->Hot_water)
                ...
                ofs << "false";
            ofs << separator;
            ofs << to_string(households[i]->numPersons) << separator << to_string(households[i]->squareMeters) << '\n';
...
int main()
{
    ...
    while (true)
    {
        ...
        else if (choice == 'p')
        {
            ...
            cout << "number of household";
            cin >> number;
            printHousehold(number_of_household[number], priceKilowattPerHour, number);
        }
        else if (choice == 'a')
        {
            for (int i = 0; i < n; i++)
            {
                if (number_of_household[i] != nullptr)
                    printHousehold(number_of_household[i], priceKilowattPerHour, i);
        ...
        else if (choice == 'r')
        {
            ...
            readDataFromFile(fileName, taskIdentifer, separator, number_of_household, n, length, house->city, priceKilowattPerHour);
        }
        else if (choice == 'w')
        {
            string fileName;
            ...
            writeDataToFile(fileName, separator, n, house->city,priceKilowattPerHour ,number_of_household);
        }
        else
        {
